    ...
    <div className=" min-h-screen">
      ...
      <div className="fixed top-24 left-4 md:left-8 z-40">
        <Link
          to="/#experience"
          data-cursor="hover"
          className="liquid-glass rounded-full px-4 py-2 inline-flex items-center gap-2 text-white/70 hover:text-white text-xs md:text-sm transition-colors"
        >
      ...
      {/* More campaigns */}
      <section className="relative px-6 py-20 md:py-28 border-t border-white/5 overflow-hidden">
        <div
          className="absolute inset-0 opacity-20"
          style={{ background: `radial-gradient(ellipse at center, ${accent}22 0%, transparent 70%)` }}
        />
        <div className="relative max-w-6xl mx-auto">
          <h3 className="text-3xl md:text-4xl text-white font-serif tracking-tight mb-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {others.map((c) => (
              <Link
                ...
                to={`/${c.slug}`}
                data-cursor="hover"
                className="group relative liquid-glass rounded-2xl p-6 overflow-hidden transition-colors hover:bg-white/[0.04]"
              >
                <div
                ...
                <div className="mt-3 text-white text-lg font-serif tracking-tight">{c.title}</div>
                <div className="mt-2 text-white/50 text-sm leading-relaxed">{c.blurb}</div>
              </Link>
            ))}
          </div>
        </div>
